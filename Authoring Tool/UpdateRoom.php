<?php
   // Configuration
	...
	$sql = "DELETE FROM RoomObjects WHERE RoomID IN 
	(SELECT RoomID FROM Rooms WHERE SceneID = $sceneID)";
	if(mysqli_multi_query($link , $sql)){
		echo "objects deleted";
	} else {
		echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
	}
	$sql ="DELETE FROM RoomTasks WHERE RoomID IN 
	(SELECT RoomID FROM Rooms WHERE SceneID = $sceneID)";
	if(mysqli_multi_query($link , $sql)){
		echo "tasks deleted";
	} else {
	...
	$sql= "DELETE FROM RoomSounds WHERE RoomID IN 
	(SELECT RoomID FROM Rooms WHERE SceneID = $sceneID)";
	if(mysqli_multi_query($link , $sql)){
		echo "sounds deleted";
	} else {
	...
	$sql = "DELETE FROM Rooms WHERE SceneID = $sceneID";
	if(mysqli_multi_query($link , $sql)){
		echo "rooms deleted";
	} else {
		echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
